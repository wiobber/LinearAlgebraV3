...
The system in Template \ref{temp:systems1} has a unique solution.  Try manipulating the system so that it has infinitely many solutions or no solutions.  Run the code to see what happens.  You will observe the following:
...
When solving systems by hand, you utilized Gauss-Jordan elimination to reduce the augmented matrix to its reduced row echelon form.  Having the reduced row echelon form available made it easy to write out the solution(s) or determine that the system is inconsistent.  We can find rref and the rank of a matrix in Octave as follows.
