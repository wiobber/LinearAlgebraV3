...
\begin{example}\label{ex:angleInOctave}
    Here we demonstrate how to find the angle between vectors $\vec{v}$ and $\vec{w}$ from the previous example.  Note the use and the syntax for the $\arccos{}$ function.
    \begin{explanation}
        \begin{verbatim}
% Define vectors v and w
v=[3;1;1;2;4];
...
\begin{example}\label{ex:orthInOctave}
    Use vectors $\vec{v}$ and $\vec{w}$ from \ref{temp:cross} to illustrate that the cross product of two vectors is orthogonal to both vectors.
    \begin{explanation}
        \begin{verbatim}
% Define vectors v and w
v=[3;1;1];
...
\begin{template}\label{temp:vectorComp}
    It is often useful to extract individual components of a vector. For example, given vector $\vec{v}=\begin{bmatrix}1\\-2\\-5\end{bmatrix}$, let's find the third component of $\vec{v}$.
\begin{verbatim}
% Define vector v
v=[1;-2;-5];
